...
class DriveSystem(object):
    ...
    def go_straight_for_inches_using_time(self, inches, speed):
        ...
        D = RT, so T=D/R or k*inches/speed, where we choose k to fit the
        given constraint. When R=10in/sec, T=1, D=10.
        When R=5in/sec (power=50), T=1, D=5.
        """
        # TODO: Test
        start_time = time.time()
        self.go(speed, speed)
        time_to_drive = inches/(speed/10)
        while True:
            current_time = time.time()
            if current_time - start_time >= time_to_drive:
                self.stop()
                return
    ...
    def go_straight_for_inches_using_encoder(self, inches, speed):
        ...
        # TODO: Test
        self.go(speed, speed)
        # time_to_drive = 360deg = self.wheel_circumference
        start_pos = self.left_motor.get_position()
        desired_pos = inches/self.wheel_circumference * 360
        while True:
            current_pos = self.left_motor.get_position()
            if abs(current_pos - start_pos) > desired_pos:
                self.stop()
                return
    ...
    def go_straight_until_intensity_is_less_than(self, intensity_target, speed):
        """
        Goes straight at the given speed until the intensity returned
        by the color_sensor is less than the given intensity.
        """
        # TODO: Test this and next one
        self.go(speed, speed)
        intensities = []
        while True:
            intensity = self.sensor_system.color_sensor.get_ambient_light_intensity()
            print(intensity)
            intensities.append(intensity)
            if len(intensities) > 5:
                intensities.pop(0)

            if statistics.mean(intensities) < intensity_target:
                self.stop()
                return

    def go_straight_until_intensity_is_greater_than(self, intensity_target, speed):
        """
        Goes straight at the given speed until the intensity returned
    ...
    def go_forward_until_distance_is_less_than(self, inches, speed):
        ...
        """
        # MB Experimented: goes below value of 20 at about 5 inches,
        # giving a multiplier of 4.
        PROXIMITY_PER_INCH = 4
        self.go(speed, speed)
        distances = []
        while True:
            ...
            if statistics.mean(distances) < inches*PROXIMITY_PER_INCH:
                self.stop()
                return
    ...
    def go_until_distance_is_within(self, delta_inches, speed):
        ...
        while True:
            ...
            if len(distances) > 5:
                distances.pop(0)
            if statistics.mean(distances) > (delta_inches+1)*PROXIMITY_PER_INCH:
                self.go(speed, speed)
            elif statistics.mean(distances) < (delta_inches-1)*PROXIMITY_PER_INCH:
                self.go(-speed, -speed)
            else:
                self.stop()
                return
...
class ArmAndClaw(object):
    ...
    def move_arm_to_position(self, desired_arm_position):
        ...
        if self.motor.get_position() < desired_arm_position:
            ...
            speed = -100
        self.motor.turn_on(speed)

        while True:
            if abs(self.motor.get_position() - desired_arm_position) < epsilon:
                self.motor.turn_off()
                return
